...
static char* readBytesWithTimeout(Client* client, size_t maxLength, size_t& dataLength, int timeout_ms)
{
  char *buf = nullptr;
  dataLength = 0;
...
bool WebServer::_parseRequest(Client* client) {
...
uint8_t WebServer::_uploadReadByte(Client* client){
  int res = client->read();
  if(res == -1){
...
bool WebServer::_parseForm(Client* client, String boundary, uint32_t len){
  (void) len;
#ifdef DEBUG_ESP_HTTP_SERVER
